                ...
                <div class="container-fluid">
                    ...
                    <div class="table-container">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($jenisKapal as $index => $data)
                                    <tr>
                                        <td class="text-center">{{ $index + 1 }}</td>
                                        <td class="text-center">{{ $data->jenis_kapal }}</td>
                                        <td class="text-center">Rp {{ number_format($data->biaya_retribusi, 0, ',', '.') }}</td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-primary btn-sm btn-edit"
                                                data-id="{{ $data->id }}"
                                                data-jenis="{{ $data->jenis_kapal }}"
                                                data-biaya="{{ $data->biaya_retribusi }}"
                                                data-bs-toggle="modal"
                                                data-bs-target="#modalEdit">
                                                <i class="fas fa-edit"></i>
                                            </button>

                                            <button type="button" class="btn btn-danger btn-sm swal-confirm" data-id="{{ $data->id }}">
                                                <i class="fas fa-trash-alt"></i>
                                            </button>

                                            <form id="delete-form-{{ $data->id }}" action="{{ route('jenis-kapal.destroy', $data->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
                        ...
                        <div class="modal-dialog">
                            <form action="{{ route('jenis-kapal.store') }}" method="POST">
                            @csrf
                            <div class="modal-content">
                                <div class="modal-header">
                                <h5 class="modal-title" id="modalTambahLabel">Tambah Jenis Kapal</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                </div>
                                <div class="modal-body">
                                <div class="mb-3">
                                    <label for="jenis_kapal" class="form-label">Jenis Kapal</label>
                                    <input type="text"autocomplete="off" class="form-control" name="jenis_kapal" required>
                                </div>
                                <div class="mb-3">
                                    <label for="biaya_retribusi" class="form-label">Biaya Retribusi</label>
                                    <input type="number" autocomplete="off" class="form-control" name="biaya_retribusi" required>
                                </div>
                                </div>
                        ...
                        <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                <h5 class="modal-title" id="modalEditLabel">Edit Jenis Kapal</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                </div>
                                <div class="modal-body">
                                <div class="mb-3">
                                    <label for="edit_jenis_kapal" class="form-label">Jenis Kapal</label>
                                    <input type="text" autocomplete="off" class="form-control" name="jenis_kapal" id="edit_jenis_kapal" required>
                                </div>
                                <div class="mb-3">
                                    <label for="edit_biaya_retribusi" class="form-label">Biaya Retribusi</label>
                                    <input type="number" autocomplete="off" class="form-control" name="biaya_retribusi" id="edit_biaya_retribusi" required>
                                </div>
                                </div>
                                <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                            ...
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                editButtons.forEach(button => {
                                    button.addEventListener('click', function () {
                                        ...
                                        document.getElementById('edit_biaya_retribusi').value = biaya;

                                        const formEdit = document.getElementById('formEdit');
                                        formEdit.action = `/jenis-kapal/${id}`;
                                    });
                                });
                            });
                        </script>
